...
def update_process_step(request):
    ...
    try:
        if step == "delete_old_resource":
            delete_request_id = delete_resource_request(resource.resource_id)
            ...
            if task_status != "SUCCESS":
                return JsonResponse({'status': 'error', 'message': f'Delete task failed with status: {task_status}'}, status=500)
            ...
            request.session['final_zip_path'] = rename_json_files_in_zip(zip_file, "media/output", resource.resource_id)
            ...
            if task_status != "SUCCESS":
                return JsonResponse({'status': 'error', 'message': f'Task failed with status: {task_status}'}, status=500)
